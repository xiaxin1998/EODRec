...
class SASRec(SequentialRecommender):
    ...
    def forward(self, item_seq, item_seq_len, mask, teacher, emb_model):
        ...
        self.one_hot = torch.zeros_like(logits).scatter_(-1, codes.unsqueeze(2), 1.0)  # k-dimensional vector
...
def train_test(model, train_data, test_data, epoch, opt, teacher, emb_model):
    ...
    for i in slices:
        ...
        emb_model.zero_grad()
        loss.backward()
        # torch.nn.utils.clip_grad_norm_(emb_model.parameters(), 0.001)
        print(loss)
