...
class SASRec(SequentialRecommender):
    ...
    def __init__(self, n_node, config):
        ...
        self.one_hot = nn.Parameter(torch.Tensor(self.n_items*config.code_book_len, config.cluster_num), requires_grad=False)
    ...
    def _init_weights(self, module):
        ...
        self.one_hot = torch.nn.Parameter(torch.Tensor(torch.load(open('../sas_stu_rr_onehot_3m_256k_test03.pkl', 'rb'))).reshape(self.n_items*self.code_book_len, self.cluster_num))
    ...
    def PredLoss(self, score_teacher, score_student):
        score_teacher = fn.softmax(score_teacher/1, dim=1)
        score_student = fn.softmax(score_student/1, dim=1)
    ...
    def forward(self, item_seq, item_seq_len, mask, teacher, emb_model):
        position_ids = torch.arange(item_seq.size(1), dtype=torch.long, device=item_seq.device)
        position_ids = position_ids.unsqueeze(0).expand_as(item_seq)
        position_embedding = self.position_embedding(position_ids)

        teacher.eval()
        logits, rec_emb = emb_model(teacher.embedding.weight)

        mse_loss = self.mse(rec_emb, teacher.embedding.weight).div(len(teacher.embedding.weight))
        self.embedding = emb_model.decoder(self.one_hot)
        del rec_emb
        self.embedding = self.eta * self.embedding + (1 - self.eta) * teacher.embedding.weight
        get = lambda i: self.embedding[item_seq[i]]
        ...
        del position_embedding, position_ids
        ...
        del trm_output, input_emb
        return self.output, self.embedding, mse_loss
    ...
    def interact(self, item_seq, item_seq_len, mask, hot_sess_items, cold_sess_items, hot_sess_len, cold_sess_len, hot_cold_tar, hot_mask, cold_mask, hot_only_index, cold_only_index, teacher, tar):
        ...
        return self.gamma*loss_pre+self.beta*con_loss
    ...
    def full_sort_predict(self, item_seq, seq_len, mask, teacher):
        ...
        del seq_output, item_emb
        return scores


def forward(model, i, data, teacher, emb_model):
    ...
    output, embedding, mse_loss = model(session_item, seq_len, mask, teacher, emb_model)
    ...
    con_loss = model.interact(session_item, seq_len, mask, hot_sess_items, cold_sess_items, hot_sess_len,
                              ...
                              teacher, tar)
    return tar, loss, con_loss, mse_loss
...
def train_test(model, train_data, test_data, epoch, opt, teacher, emb_model):
    print('start training: ', datetime.datetime.now())
    total_loss = 0.0
    set_seed(opt)
    slices = train_data.generate_batch(opt.batch_size)
    emb_model.train()
    optimizer = torch.optim.Adam(emb_model.parameters(), lr=0.0001)
    for i in slices:
        tar, loss, con_loss, mse_loss = forward(model, i, train_data, teacher, emb_model)
        loss = loss + mse_loss + con_loss
        model.zero_grad()
        emb_model.zero_grad()
        loss.backward()
